...
description: |
  爆款工厂：把"别人的爆款短视频"变成"你自己口吻的成片 + 全平台文案"的一条龙入口。串起四件事——①女娲蒸馏你自己的表达DNA（一次性）②video-distill 蒸馏对标爆款→拆解/翻拍角度 ③hyperframes 把翻拍稿做成成片 ④四平台爆款文案。它是个路由器/checklist，按顺序调度 nuwa-skill / video-distill / hyperframes，并在翻拍和文案两处注入你自己的 profile。
  什么时候用（要 pushy 一点，别漏触发）：团队新人 onboarding、"我要开始做爆款翻拍"、"搭一下翻拍流程"、"装一下这套 skill"、"我想翻拍这条视频"（给了对标视频链接/文件 + 想做成自己的成片或文案时）、"蒸馏我自己"、"从对标视频到成片到文案，帮我走一遍"。只要用户想把一条对标视频变成自己的内容产物，就用这个 skill 当总入口；具体某一步的细节再交给对应子 skill。
---
