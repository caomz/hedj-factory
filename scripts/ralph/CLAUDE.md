...
以下文件都在scripts/ralph下: prd.json（必存在，缺失即 blocker）、progress.txt（仅追加日志，首次启动可为不存在）。

## 你的任务

1. 读取 `prd.json` 中的 PRD（与此文件在同一目录）
2. 读取 `progress.txt` 中的进度日志（若文件不存在则视为空日志，仅含 `## Codebase Patterns` 段；按下方模板初始化后继续，不要因此报 blocker）。先看 `## Codebase Patterns` 部分。
...
9. 每次完成运行后, 将你的进度追加到 `progress.txt`（若文件不存在则先 `touch` 或 `Write` 初始化一份仅含 `## Codebase Patterns` header 的模板，再追加）。

## 进度报告格式

追加到 progress.txt（永远不要替换，始终追加；若文件不存在则先初始化为只含 `## Codebase Patterns` header 的模板，再追加）：
```
...
如果你发现未来迭代应该知道的**可重用 pattern**，将其添加到 progress.txt 顶部的 `## Codebase Patterns` 部分（如果不存在则创建该节；如果整个文件不存在也按上面第 9 步先初始化）。此部分应整合最重要的学习：
...
- 频繁提交
- 保持 CI 绿色
- 在开始之前阅读 progress.txt 中的 Codebase Patterns 部分（若文件不存在则跳过本步）。

## 关于该项目的重要注意事项

项目跟路径下读取AGENTS.md, 这是整个项目的技术架构开发指导说明, 也就是harness.

先加载这些“补充说明信息.md”。我要做的这些 story 都来自项目根路径下 `tasks/` 中与本次 Ralph 任务对应的 PRD 源文件 —— 即与 `scripts/ralph/prd.json` 的 `branchName` 对应的那份 `tasks/prd-<feature>.md`(去掉 `ralph/` 前缀、加 `prd-` 前缀；例如 branchName 为 `ralph/foo` 时对应 `tasks/prd-foo.md`)。如果开发过程中有需求不明确的地方，可以去那份 PRD 查看。
